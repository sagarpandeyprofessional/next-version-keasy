...
export function PaymentCheckoutPage() {
    ...
    switch (selectedPaymentMethod) {
      case "CARD":
        await payment.requestPayment({
          ...
          successUrl: window.location.origin + "/toss/payment/success", // 결제 요청이 성공하면 리다이렉트되는 URL
  ...
  async function requestBillingAuth() {
    await payment.requestBillingAuth({
      ...
      successUrl: window.location.origin + "/toss/payment/billing", // 요청이 성공하면 리다이렉트되는 URL
      failUrl: window.location.origin + "/toss/fail", // 요청이 실패하면 리다이렉트되는 URL
      customerEmail: "customer123@gmail.com",
      customerName: "김토스",
    });
  }
