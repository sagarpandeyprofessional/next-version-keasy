...
export default function GuidePost() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!user) { alert('Sign in to create guides'); router.push('/guides'); return; }
      setUser(user);
      setLoading(false);
    } catch (error) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      router.push(`/guides/guide/${data.id}`);
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-8 px-4">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => router.push('/guides')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6">
