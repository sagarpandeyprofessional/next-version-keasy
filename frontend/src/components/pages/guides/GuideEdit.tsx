...
export default function GuideEdit() {
  ...
  const checkAuthAndLoadGuide = async () => {
    try {
      ...
      if (!user) { alert('Sign in to edit guides'); router.push('/guides'); return; }
      setUser(user);
  ...
  const loadGuide = async (userId) => {
    try {
      ...
      if (!data) { alert('Guide not found'); router.push('/guides'); return; }
      if (data.created_by !== userId) { alert('You can only edit your own guides'); router.push('/guides'); return; }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Main Content Area */}
      <div className={`transition-all duration-300 ease-in-out ${sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-64'} ${showFormattingToolbar ? 'pt-[48px]' : ''}`}>
